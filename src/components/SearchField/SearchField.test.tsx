...
describe('Autocomplete mode', () => {
  ...
  test('autocomplete options appear on user input', () => {
    getAutocompleteComponent({ mode: 'autocomplete' });
  ...
  test('clear button works in autocomplete mode', () => {
    getAutocompleteComponent({ mode: 'autocomplete' });
    const search = screen.getByLabelText(testLabel);
    userEvent.type(search, 'clear');
    expect(search).toHaveValue('clear');
  ...
  test("No popup when there aren't any options", () => {
    getAutocompleteComponent({ mode: 'autocomplete' });
    const search = screen.getByLabelText(testLabel);
    act(() => { search.focus(); });
    expect(search).toHaveFocus();
