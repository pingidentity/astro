...
import {
  PaginationProps,
  RangeSelectPopoverProps,
} from '../../types/pagination';

const RangeSelectPopover = forwardRef<HTMLDivElement, RangeSelectPopoverProps>(
  (props, ref) => {
    const { state, buttonProps } = props;

    const { setOffsetCount, offsetOptions, ...others } = state;
      ...
      <OverlayProvider>
        <Box sx={{ flexGrow: 1 }}>
          <PopoverMenu ref={menuRef}>
            <Button {...buttonProps} variant="paginationMenu">
              {state.popoverButtonString}
            </Button>
            <Menu
...
const Pagination = forwardRef<HTMLDivElement, PaginationProps>((props, ref) => {
  const { previousButtonIconProps, nextButtonIconProps } = props;
