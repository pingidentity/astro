...
}, new go.Binding('location', 'loc', go.Point.parse).makeTwoWay(go.Point.stringify), new go.Binding('click', 'onClick'), $(go.Panel, 'Auto', { name: 'BODY' }, $(go.Shape, 'RoundedRectangle', { fill: 'transparent', strokeWidth: 0 }), new go.Binding('minSize', '', s => getIfLengthGreater(s.errorMessage, getSize(s, 'errorContainer'), getSize(s, ''), 0)), $(go.Panel, 'Vertical', { padding: 15, alignment: go.Spot.Top }, new go.Binding('visible', '', s => getIfLengthGreater(s.errorMessage, true, false, 0)), $(go.Picture, {
...
), $(go.Panel, 'Position', { position: new go.Point(0, 0) }, { name: 'BODY' }, $(go.Shape, 'RoundedRectangle', { fill: 'transparent', stroke: 'transparent', parameter1: 3, margin: new go.Margin(30, 0, 0, 0) }, new go.Binding('desiredSize', '', s => getSize(s, 'transparentContainer')),
...
), $(go.Panel, 'Horizontal', { alignment: go.Spot.Left }, $(go.Panel, 'Auto', $(go.Shape, 'StepIconBG', { stroke: 'transparent', strokeWidth: 0, margin: new go.Margin(30, 0, 0, 10), parameter1: 2 }, new go.Binding('desiredSize', '', s => getSize(s, 'shape')), new go.Binding('fill', 'color')), $(go.Picture,
    { width: 20, height: 20, margin: new go.Margin(0, 0, 0, -9) },
    new go.Binding('source', 'getIconSrc', getIcon(color || COLORS.WHITE)),
),
), $(go.Panel, 'Vertical', { margin: new go.Margin(30, 0, 0, -5) }, $(go.TextBlock,
    {
...
), iconButton({ onAction: onDelete, margin: new go.Margin(90, 20, 0, 0), source: svgComponentToBase64(<Icon path={mdiDelete} height="20px" width="20px" color={COLORS.WHITE} />) }), fromNode(), toNode(), bottomNode(),
),
);
