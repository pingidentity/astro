...
module.exports = function (state, action) {
    ...
    switch (action.type) {
        ...
        case Actions.Types.LIST_VIEW_PAGE:
            ...
            nextState = update.set(nextState, ["rows", "page"], pageSelector(nextState));
            break;
        case Actions.Types.LIST_VIEW_REORDER:
            const record = nextState.rows.all[action.from];
            const destination = action.from < action.to ? action.to - 1 : action.to;
            nextState = update.splice(nextState, ["rows", "all"], action.from, 1);
            nextState = update.splice(nextState, ["rows", "all"], destination, 0, record);
            nextState = update.set(nextState, ["rows", "filtered"], filterSelector(nextState));
            nextState = update.set(nextState, ["rows", "page"], pageSelector(nextState));
            break;
        default:
